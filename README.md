...
- **Send Newsletter** — Bulk-send a template (or plain `{{token}}` message) to a CSV of recipients, immediately or on a schedule. Runs as a paced background job (per-recipient render, progress tracking, pause/resume/cancel) — see [the docs](docs/sending-email-with-templates.md#newsletters-bulk-send).
