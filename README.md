...
- **HTML templates** — Per-mailbox reusable templates with `{{placeholder}}` tokens (text or HTML fragments) and inline images. Body editor has three modes: rich-text, a drag-and-drop **Design** builder (GrapesJS newsletter preset, lazy-loaded), and raw **HTML** (paste full table-based templates). Available in the composer, the send/reply/forward APIs (`template_id` + `placeholders`), and MCP (`list_templates` / `get_template`, plus `templateId` on the send/draft tools). Template images are served from the `/assets/t/<mailbox>/<assetId>` route so recipients' mail clients can load them; that route must be reachable without a Cloudflare Access login (set `ASSET_URL` to a dedicated no-Access host, or bypass the path) — see [the docs](docs/sending-email-with-templates.md#uploading-images).
- **Send Newsletter** — Bulk-send a template (or plain `{{token}}` message) to a CSV of recipients or to selected **mail lists**, immediately or on a schedule. Runs as a paced background job (per-recipient render, progress tracking, pause/resume/cancel) — see [the docs](docs/sending-email-with-templates.md#newsletters-bulk-send).
- **Contacts & Mail Lists** — A per-mailbox address book: import contacts from CSV, group them into mail lists, pick them in the composer (type-ahead + browse), and target lists from a newsletter — see [the docs](docs/sending-email-with-templates.md#contacts--mail-lists).
...
Any user who passes the shared Cloudflare Access policy can access all mailboxes in this app by design. This includes the MCP server at `/mcp` -- external AI tools (Claude Code, Cursor, etc.) connected via MCP can operate on any mailbox by passing a `mailboxId` parameter. There is no per-mailbox authorization; the Cloudflare Access policy is the single trust boundary. Template bodies and placeholder values are treated as trusted HTML authored by an authenticated user (the same posture as the per-mailbox agent system prompt) and are not server-side sanitized. Template image bytes are served without Access at `/assets/t/<mailbox>/<assetId>` (so they load in sent email) — reachable only via the unguessable asset UUID. On a dedicated `ASSET_URL` host the Worker serves that route and nothing else.
